...
/* Disposable saved-world crash recovery: kills only this fixture process group, then reopens its isolated state. */
...
try {
  ...
  writeFileSync(testRepo+'/tools/proposal-test.mjs',"import test from 'node:test';test('wait for crash',async()=>{await new Promise(r=>setTimeout(r,20000));});");
  await click('#attempt-run-'+attempt.id);
  const unfinished=await until(()=>script(`return Object.values(window.cockpit.frame.projection.development_attempts[arguments[0]].test_runs).find(r=>r.state==='started')`,[attempt.id]));
  check('unfinished run records its owning runtime identity',typeof unfinished.runtime_epoch==='string');
  await until(()=>script(`return !!document.querySelector('[data-test-run="'+arguments[0]+'"] [data-cancel-test]')`,[unfinished.run_id]));
  await script(`document.querySelector('[data-test-run="'+arguments[0]+'"]').scrollIntoView({block:'center'})`,[unfinished.run_id]);evidence.capture('Unfinished test before deliberate app crash');
  ...
  await wd('POST',`/session/${session}/window/rect`,{width:1280,height:850});
  check('app crash reopens the same saved world with a new runtime',await script(`const w=window.cockpit.frame.world;return w.world_incarnation===arguments[0].world_incarnation&&w.world_generation===arguments[0].world_generation&&w.projection_epoch!==arguments[0].projection_epoch`,[beforeWorld]));
  ...
  await until(()=>script(`return window.cockpit.frame.projection.development_attempts[arguments[0]].test_runs[arguments[1]].outcome?.reason==='interrupted'`,[attempt.id,unfinished.run_id]));
  await until(()=>script(`return document.querySelector('#attempt-test-history-'+arguments[0]).textContent.includes('No passing result is recorded')`,[attempt.id]));
  check('crashed test becomes a durable interrupted result without a passing verdict',await script(`const r=window.cockpit.frame.projection.development_attempts[arguments[0]].test_runs[arguments[1]];return r.state==='failed'&&r.revision===2&&r.outcome.verdict===null&&r.outcome.test_count===0`,[attempt.id,unfinished.run_id]));
  check('restart preserves the earlier completed test and does not rerun tests',await script(`const runs=Object.values(window.cockpit.frame.projection.development_attempts[arguments[0]].test_runs);return runs.length===2&&runs.filter(r=>r.outcome?.verdict===arguments[1]).length===1&&!runs.some(r=>r.state==='started')`,[attempt.id,expectedTextOutcome]));
  await script(`document.querySelector('[data-test-run="'+arguments[0]+'"]').scrollIntoView({block:'center'})`,[unfinished.run_id]);evidence.capture('App restart records interrupted tests with no passing verdict');
  await reloadPage();await click('#app-navigation [data-nav=development-tasks]');await click('#development-task-list article button');await click('[data-attempt-id="'+attempt.id+'"] > summary');
  await until(()=>script(`return document.querySelector('#attempt-test-history-'+arguments[0]).textContent.includes('No passing result is recorded')`,[attempt.id]));
  check('reopening the review preserves its recovered interruption',await script(`return window.cockpit.frame.projection.development_attempts[arguments[0]].test_runs[arguments[1]].outcome.reason==='interrupted'`,[attempt.id,unfinished.run_id]));
  evidence.complete();console.log(`development crash smoke: ${checks} held`);
