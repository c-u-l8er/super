...
import { mkdirSync, writeFileSync, readFileSync, mkdtempSync, rmSync, statSync, existsSync } from 'node:fs';
...
const evidence=visualEvidence('change-set-apply-smoke',root,driver);
...
try {
  ...
  writeFileSync(testRepo+'/style.css','External edit after staging\n');await click('#editor-set-apply');
  await until(()=>script(`return document.querySelector('#editor-status').textContent.includes('changed')`));
  check('apply refuses a changed second member without saving the first',readFileSync(testRepo+'/index.html','utf8')==='<h1>Before task</h1>\n'&&readFileSync(testRepo+'/style.css','utf8')==='External edit after staging\n');photo('06_Apply_Conflict');
  writeFileSync(testRepo+'/style.css','h1 { color: red; }\n');await click('#editor-set-apply');
  await until(async()=>readFileSync(testRepo+'/index.html','utf8')===proposedText&&readFileSync(testRepo+'/style.css','utf8')==='h1 { color: blue; }\n'&&await script(`return !document.querySelector('[data-screen=editor] .workbench-tabs').textContent.includes('●')`));
  check('editor retains usable width with the apply notice',await script(`return document.querySelector('#editor-content').getBoundingClientRect().width>300`));
  check('one apply saves both reviewed files and clears both dirty markers',!existsSync(testRepo+'/.git/super-apply-journal-v1.json'));photo('07_Whole_Set_Applied');
  check('application does not create test or acceptance claims',await script(`return Object.keys(window.cockpit.frame.projection.development_attempts??{}).length===0`));
  // Reconstruct the durable boundary after first-file write, then restart the
  // actual app. This is explicit recovery-fixture material, not a timed crash.
  const info=statSync(testRepo),material={version:1,device:info.dev,inode:info.ino,files:[{path:'index.html',original:'<h1>Before task</h1>\n',content:proposedText},{path:'style.css',original:'h1 { color: red; }\n',content:'h1 { color: blue; }\n'}]};
  writeFileSync(testRepo+'/.git/super-apply-journal-v1.json',JSON.stringify(material));writeFileSync(testRepo+'/style.css',material.files[1].original);
  ...
  await until(()=>script('return !!window.cockpit?.frame?.projection'));await click('[data-rail-mode=nav]');await click('#app-navigation [data-nav=editor]');
  await click('#development-choose-editor');await sleep(800);run('/usr/bin/python3',[`${root}/tools/development-confirm-folder.py`,String(driver.pid),testRepo]);
  await until(()=>script(`return !!document.querySelector('#editor-set-finish')&&!document.querySelector('#editor-set-finish').disabled`));
  check('native restart exposes the pending file set and both recovery choices',await script(`return document.querySelector('#editor-change-set').textContent.includes('index.html')&&!!document.querySelector('#editor-set-restore')`));photo('08_Restart_Recovery');
  await wd('POST',`/session/${session}/window/rect`,{width:1000,height:760});
  check('recovery choices remain visible in a narrower window',await script(`return document.querySelector('#editor-set-restore').getBoundingClientRect().right<=innerWidth`));photo('09_Narrow_Recovery');
  await wd('POST',`/session/${session}/window/rect`,{width:1280,height:850});
  await click('#editor-set-restore');await until(()=>script(`return !!document.querySelector('#editor-set-confirmation')?.open`));photo('Recovery_confirmation');await click('#editor-set-cancel');check('cancelling recovery preserves the partial result and journal',existsSync(testRepo+'/.git/super-apply-journal-v1.json')&&readFileSync(testRepo+'/index.html','utf8')===proposedText);
  writeFileSync(testRepo+'/style.css','Unrelated change\n');await click('#editor-set-finish');await click('#editor-set-confirm');
  await until(()=>script(`return document.querySelector('#editor-status').textContent.includes('changed on disk')`));
  check('recovery refuses unrelated edits and keeps its journal',existsSync(testRepo+'/.git/super-apply-journal-v1.json')&&readFileSync(testRepo+'/style.css','utf8')==='Unrelated change\n');photo('10_Recovery_Conflict');
  writeFileSync(testRepo+'/style.css',material.files[1].original);await click('#editor-set-finish');await click('#editor-set-confirm');
  await until(()=>!existsSync(testRepo+'/.git/super-apply-journal-v1.json'));
  check('finish recovery completes the second file and removes the journal',readFileSync(testRepo+'/style.css','utf8')===material.files[1].content);photo('11_Recovery_Finished');
  writeFileSync(testRepo+'/.git/super-apply-journal-v1.json',JSON.stringify(material));writeFileSync(testRepo+'/style.css',material.files[1].original);
  await reloadPage();await click('[data-rail-mode=nav]');await click('#app-navigation [data-nav=editor]');await until(()=>script(`return !!document.querySelector('#editor-set-restore')&&!document.querySelector('#editor-set-restore').disabled`));
  await click('#editor-set-restore');await click('#editor-set-confirm');await until(()=>!existsSync(testRepo+'/.git/super-apply-journal-v1.json'));
  check('restore recovery returns both original files',readFileSync(testRepo+'/index.html','utf8')===material.files[0].original&&readFileSync(testRepo+'/style.css','utf8')===material.files[1].original);photo('12_Originals_Restored');
  evidence.complete();console.log(`recoverable change-set smoke: ${checks} held`);
