...
try {
  ...
  check('combined staging creates no single-file test or acceptance claims',await script(`return Object.values(window.cockpit.frame.projection.development_attempts??{}).every(a=>!a.test_runs&&!a.acceptance)`));
  ...
  const noted=await script(`return window.cockpit.frame.projection.development_attempts[arguments[0]]`,[savedSet.id]);assert.deepEqual(noted.files,savedSet.files);check('combined review notes preserve the immutable source and result',noted.source.result_sha256===savedSet.source.result_sha256&&noted.status==='needs_changes');
  ...
  await wd('POST',`/session/${session}/window/rect`,{width:1280,height:850});
  check('actual process restart preserves the set with a new runtime epoch',await script(`const w=window.cockpit.frame.world,a=window.cockpit.frame.projection.development_attempts[arguments[1].id];return w.world_incarnation===arguments[0].world_incarnation&&w.projection_epoch!==arguments[0].projection_epoch&&a.revision===2&&a.history.at(-1).note.includes('CSS contrast')`,[beforeWorld,savedSet]));
  assert.deepEqual((await script(`return window.cockpit.frame.projection.development_attempts[arguments[0]]`,[savedSet.id])).files,savedSet.files);
  await click('#app-navigation [data-nav=development-tasks]');await click('#development-task-list article button');await click('[data-attempt-id="'+savedSet.id+'"] > summary');
  await script(`document.querySelector('[data-attempt-id="'+arguments[0]+'"] ').scrollIntoView({block:'center'})`,[savedSet.id]);photo('08_After_App_Restart');
  check('restart does not send provider messages or fabricate acceptance',chatCalls===callsBefore&&await script(`const a=window.cockpit.frame.projection.development_attempts[arguments[0]];return !a.acceptance&&!a.test_runs`,[savedSet.id]));
  console.log(`durable change-set smoke: ${checks} held`);
