...
let driver = spawn('tauri-driver', ['--port', String(port), '--native-port', String(port + 1), '--native-driver', '/usr/bin/WebKitWebDriver'], {
  ...
  env: { ...process.env, XDG_DATA_HOME:testData, AMPD_DIR: `${root}/ampd`, XDG_STATE_HOME:testData+'/state', SUPER_WORLD_MODE: 'saved', SUPER_WORLD:'conversation-fixture', SUPER_COCKPIT_FIXTURE: '0', SUPER_COCKPIT_CARRIER: '0', SUPER_COCKPIT_PANE: '0', WEBKIT_DISABLE_COMPOSITING_MODE: '1' },
});
...
try {
  ...
  check('conversation return link identifies the shared plan',await script(`return document.querySelector('.task-conversation-backlink').dataset.developmentTask===arguments[0]`,[task.id]));
  await photo('01_Conversation_Link','.task-conversation-backlink');
  await click('.task-conversation-backlink');
  check('task shows the linked reply while provider is waiting',await script(`return document.querySelector('#task-provider-session').textContent.includes('Waiting for reply')`));
  await photo('02_Task_Waiting','#task-provider-session');
  await until(()=>script(`return document.querySelector('#task-provider-session').textContent.includes('Reply received')`));
  check('task receives completed reply state without changing plans',await script(`return document.querySelector('#task-provider-session').textContent.includes('Reply received')&&window.cockpit.frame.projection.development_tasks[arguments[0]].status==='blocked'`,[task.id]));
  ...
  await reloadPage();await click('#app-navigation [data-nav=development-tasks]');await click('#development-task-list article button');
  check('reload does not restore live task reply',await script(`return !document.querySelector('#task-provider-session').textContent.includes('Waiting for reply')&&!document.querySelector('#task-provider-session').textContent.includes('Reply received')`));
  await wd('POST',`/session/${session}/window/rect`,{width:1000,height:760});
  await photo('05_Reload_Narrow','#task-provider-session');

  const linkedId=await script(`return document.querySelector('[data-task-conversation]').dataset.taskConversation`);
  check('task lists a persisted conversation',!!linkedId);
  await photo('06_Saved_Conversation_List','#task-provider-session');
  ...
  await until(()=>script(`return !!window.cockpit?.frame?.projection?.development_tasks?.[arguments[0]]`,[task.id]));
  await wd('POST',`/session/${session}/window/rect`,{width:1280,height:850});
  check('actual app restart preserves world with a new runtime epoch',await script(`const w=window.cockpit.frame.world;return w.world_incarnation===arguments[0].world_incarnation&&w.world_generation===arguments[0].world_generation&&w.projection_epoch!==arguments[0].projection_epoch`,[beforeWorld]));
  await click('#app-navigation [data-nav=development-tasks]');await click('#development-task-list article button');
  check('linked conversation remains after process restart',await script(`return document.querySelector('[data-task-conversation]').dataset.taskConversation===arguments[0]`,[linkedId]));
  await photo('08_After_App_Restart','#task-provider-session');
  await click('[data-task-conversation="'+linkedId+'"]');
  await until(()=>script(`return document.querySelector('#bot-history').value===arguments[0]&&document.querySelector('#bot-surface').textContent.includes('Saved proposal history · no action is restored')`,[linkedId]));
