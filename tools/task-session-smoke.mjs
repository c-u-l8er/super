...
try {
  ...
  console.log(`task session smoke: ${checks} held`);
} catch(e){console.error(log);throw e;} finally {evidence.close();botFixture.close();if(session)try{await wd('DELETE',`/session/${session}`);}catch{}try{process.kill(-driver.pid,'SIGTERM');}catch{}}
