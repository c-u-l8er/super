...
const botFixture = createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    if(lastBotRequest.messages.at(-1).content.includes('Make the planned edit')){setTimeout(()=>res.end(JSON.stringify({message:{role:'assistant',content:'Review the planned change.',tool_calls:[{function:{name:'propose_file_edit',arguments:{path:'index.html',content:proposedText}}}]},done:true})),8000);return;}
...
try {
  ...
  check('editor return link is bound to this plan',await script(`return document.querySelector('#editor-return-task').dataset.developmentTask===arguments[0]`,[task.id]));
  ...
  await click('#editor-return-task');
  check('task shows current repository and selected tab',await script(`const e=document.querySelector('#task-editor-context');return e.textContent.includes(arguments[0])&&e.textContent.includes('No draft changes')&&e.querySelector('[data-task-editor-file="index.html"]')`,[testRepo]));
  await photo('01_Task_Editor_Context','#task-editor-context');
  await click('[data-task-editor-file="index.html"]');
  check('task file link returns to selected editor tab',await script(`return !document.querySelector('[data-screen=editor]').hidden&&document.querySelector('#editor-status').textContent.includes('index.html')`));
  await click('#editor-content .cm-content');await type('#editor-content .cm-content','Draft only');
  await until(()=>script(`return !document.querySelector('#editor-save').disabled`));
  await click('#editor-return-task');
  check('unsaved editor changes appear without saving disk',readFileSync(testRepo+'/index.html','utf8')==='<h1>Before task</h1>\n'&&await script(`return document.querySelector('#task-editor-context').textContent.includes('Unsaved draft')`));
  await photo('02_Unsaved_Draft','#task-editor-context');
  await script(`const e=document.querySelector('#task-status');e.value='planned';e.dispatchEvent(new Event('change',{bubbles:true}))`);
  await type('#task-note','Continue with updated plan context');await click('#development-plan-update button');
  await until(()=>script(`return window.cockpit.frame.projection.development_tasks[arguments[0]].revision===3`,[task.id]));
  check('changed plan invalidates file links',await script(`return document.querySelector('#task-editor-context').textContent.includes('earlier or closed plan')&&!document.querySelector('[data-task-editor-file]')`));
  await photo('03_Changed_Plan','#task-editor-context');
  await click('#task-prepare-file');await click('#editor-return-task');
  check('explicit preparation reconnects existing unsaved tab',await script(`return document.querySelector('#task-editor-context').textContent.includes('Unsaved draft')&&!!document.querySelector('[data-task-editor-file="index.html"]')`));
  await click('[data-task-editor-file="index.html"]');await click('#editor-clear-task');
  await click('[data-rail-mode=nav]');await click('#app-navigation [data-nav=development-tasks]');
  check('clearing context removes task file links',await script(`return document.querySelector('#task-editor-context').textContent.includes('No current Editor session')&&!document.querySelector('[data-task-editor-file]')`));
  await click('#task-prepare-file');await click('#editor-return-task');await reloadPage();
  await click('#app-navigation [data-nav=development-tasks]');await click('#development-task-list article button');
  check('reload does not restore live Editor linkage',await script(`return document.querySelector('#task-editor-context').textContent.includes('No current Editor session')&&!document.querySelector('[data-task-editor-file]')`));
  await wd('POST',`/session/${session}/window/rect`,{width:1000,height:760});await photo('04_Reload_Narrow','#task-editor-context');
  check('navigation and draft inspection never sent provider messages',chatCalls===0);
  console.log(`task editor smoke: ${checks} held`);
} catch(e){console.error(log);if(session)try{console.error(await script(`return document.querySelector('#workspace-canvas').innerText.slice(-2000)`));}catch{}throw e;} finally {evidence.close();botFixture.close();if(session)try{await wd('DELETE',`/session/${session}`);}catch{}try{process.kill(-driver.pid,'SIGTERM');}catch{}}
